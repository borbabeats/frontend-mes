...
Key domain facts to consider when relevant to a principle:
- **Roles**: ADMIN, GERENTE (manager), OPERADOR (operator), PLANEJAMENTO (planning) — each with distinct permissions.
...
- **Apontamentos** are always linked to an OP and a machine, auto-assigned to the logged-in operator, and track produced/defective quantities — data accuracy here is business-critical.
- **Dashboard KPIs**: OEE (Availability × Performance × Quality), overall efficiency, and defect rate are computed from production data and drive management decisions.
- **Deployment constraint**: The app is statically exported for AWS S3 hosting — no SSR/dynamic server routes; all data fetching is client-side.

Read `README.MD` and `DOCUMENTACAO_PROJETO.md` at the repository root for full architecture, tech stack, and business rule details before drafting or amending principles.
